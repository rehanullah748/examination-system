...
const Nav = ({auth}) => {
  console.log(auth)
    ...
    const { error, isError, isSuccess, isLoading, mutate, data} = useMutation(() => {
        return axios.post('http://localhost:5001/api/auth/logout', {}, {withCredentials:true})
      })
      const logOut = (e) => {
        e.preventDefault()
        mutate({})
  ...
  return (
    ...
    <nav className="flex basis-full items-center w-full mx-auto px-4 sm:px-6 md:px-8" aria-label="Global">
      <div className="me-5 lg:me-0 lg:hidden">
        <Link className="flex-none text-xl font-semibold dark:text-white" href='/'>Home</Link>
      </div>
        ...
        <div className="flex flex-row items-center justify-end gap-2">
          ...
          <div className="hs-dropdown relative inline-flex [--placement:bottom-right]">
            <button id="hs-dropdown-with-header" type="button" className="w-[2.375rem] h-[2.375rem] inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-full border border-transparent text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
              {auth?.user?.image ? <img className="inline-block h-[2.375rem] w-[2.375rem] rounded-full ring-2 ring-white dark:ring-gray-800" src={auth?.user?.image} alt="Image Description"/> : <span className="h-[2.375rem] flex justify-center items-center bg-blue-600 text-white text-sm uppercase  font-semibold rounded-full w-[2.375rem]">{auth?.user?.name[0]}</span>}
            </button>
            ...
            <div className="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[15rem] bg-white shadow-md rounded-lg p-2 dark:bg-gray-800 dark:border dark:border-gray-700" aria-labelledby="hs-dropdown-with-header">
              ...
              <div className="mt-2 py-2 first:pt-0 last:pb-0">
                ...
                <Link href='/dashboard/setting' className="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300">
                ...
                {auth.auth && <a onClick={logOut} className="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300" href="#">
                  <svg className="flex-shrink-0 w-4 h-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                  Logout
                </a>}
